...
export class SaleOrderComponent implements OnInit {
  ...
  charge: Charge = {
    ...
    color: 'primary',
    subcharges: [
      {name: 'Avec Emballage', completed: false, color: 'primary'},
      {name: 'Avec Serviette de table', completed: false, color: 'accent'},
      {name: 'Avec couvert', completed: false, color: 'warn'}
    ]
  };
  foods = [
  {
  ...
  updateAllComplete() {
    this.allComplete = this.charge.subcharges != null && this.charge.subcharges.every(t => t.completed);
  }

  someComplete(): boolean {
    if (this.charge.subcharges == null) {
      return false;
    }
    return this.charge.subcharges.filter(t => t.completed).length > 0 && !this.allComplete;
  }
  ...
  setAll(completed: boolean) {
    ...
    this.charge.subcharges.forEach(t => t.completed = completed);
  }
  validateOrder(){
